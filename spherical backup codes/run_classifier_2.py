...
######################################################################
######################################################################
def train_random_forest_and_evaluate(X, y, seed,max_tree_depth=15):
    np.random.seed(seed)  # For reproducibility of random splits
    accuracies = []  # To store accuracy of each split
    ...
    for _ in range(20):  # Perform 20 random splits and evaluations
        ...
        model = RandomForestClassifier(max_depth=max_tree_depth,n_estimators = 100)
        model.fit(X_train, y_train)
        #n_trees = model.min_impurity_decrease
        #print(f"The number of max_samples in the model: {n_trees}")
        #print(max_tree_depth)
...
results_filename = os.path.join(directory, "classification_results_rf2.csv")
if not os.path.exists(results_filename):
    ...
    for d in range(D_,D_+2):
        ...
        for func in functions:
            ...
            #rounded_array = np.round(D*X_.T**2, decimals=3)
