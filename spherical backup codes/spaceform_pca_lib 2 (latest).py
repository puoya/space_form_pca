...
def estimate_spherical_subspace(X,param):
    ...
    # product = np.matmul(X.T,p)
    # np.clip(product, -1, 1, out=product)
    # distortion1 = np.mean(np.arccos(product))
    # distortion2 = np.mean(np.arccos(-product))
    # if distortion2 > distortion1:
    #     S.p = -p
